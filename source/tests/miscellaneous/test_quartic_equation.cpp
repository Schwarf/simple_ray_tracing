...
{
	FixedSizedArray<5, float> coefficients{a1_, b1_, c1_, d1_, e1_};
...
{
	FixedSizedArray<5, float> coefficients{a2_, b2_, c2_, d2_, e2_};
	auto solver = QuarticEquation<float>(coefficients, epsilon_);
...
{
	FixedSizedArray<5, float> coefficients{a3_, b3_, c3_, d3_, e3_};
	auto solver = QuarticEquation<float>(coefficients, epsilon_);
	EXPECT_EQ(solver.number_of_solutions(), 2);
...
{
	FixedSizedArray<5, float> coefficients{a4_, b4_, c4_, d4_, e4_};
	auto solver = QuarticEquation<float>(coefficients, epsilon_);
	EXPECT_EQ(solver.number_of_solutions(), 4);
...
{
	FixedSizedArray<5, float> coefficients{a5_, b5_, c5_, d5_, e5_};
	auto solver = QuarticEquation<float>(coefficients, epsilon_);
	EXPECT_EQ(solver.number_of_solutions(), 2);
...
{
	FixedSizedArray<5, float> coefficients{a6_, b6_, c6_, d6_, e6_};
	auto solver = QuarticEquation<float>(coefficients, epsilon_);
