...
__device__ void build_material( IMaterial * p_material)
{
	...
	FloatTriple color = FloatTriple{0.9, 0.2, 0.3};
	p_material->set_rgb_color(color);
}

__global__ void render_it(FloatTriple *buffer, size_t max_width, size_t max_height)
{
	...
	auto sphere_center = FloatTriple{-3.5f, 3.5f, -15.f};
	...
	FloatTriple direction = FloatTriple{x_direction, y_direction, z_direction}.normalize();
	FloatTriple origin = FloatTriple{0, 0, 0};
	auto ray = Ray(origin, direction);

	size_t pixel_index = height * max_width + width;
	FloatTriple hit_normal = FloatTriple{0, 0, 0};
	FloatTriple hit_point = FloatTriple{0, 0, 0};

	if(sphere.does_ray_intersect(ray,hit_normal, hit_point)) {
		buffer[pixel_index] = sphere.material()->rgb_color();
	}
	else
		buffer[pixel_index] = FloatTriple{0.2, 0.7, 0.8};
}
